...
export function createQuery(options) {
    ...
    const preloadEntry = staticKey ? queryCache.getEntry(staticKey) : null;
    ...
    const dataSignal = signal(preloadData);
     ...
     */
    async function executeFetch(serializedKey, background = false) {
        if (isDisposed)
            return;
        // Check if enabled
        if (!opts.enabled()) {
     ...
     */
    function onFocus() {
        if (isDisposed || !opts.refetchOnFocus)
            return;
     ...
     */
    function initialize(serializedKey) {
        ...
        const cachedEntry = queryCache.getEntry(serializedKey);
